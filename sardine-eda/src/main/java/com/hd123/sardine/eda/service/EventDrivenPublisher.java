...
import static com.hd123.sardine.eda.api.EventStatus.*;
...
public class EventDrivenPublisher {
    ...
    @Autowired
    private EventPublisherDao publisherMapper;
    @Autowired
    ...
    public int persistPublishMessage(Object payload, String businessType) {
        ...
        publisher.setPayload(SerializationUtils.parse(payload));
        publisher.setBussinessType(businessType);
    ...
    public void fetchAndPublishToBroker(BatchFetchEventStrategy fetchStrategy) {
        ...
        for (EventPublisher event : events) {
            final String type = event.getBussinessType();
    ...
    private class RabbitReturnCallback implements RabbitTemplate.ReturnCallback {
        ...
        public void returnedMessage(Message message, int replyCode, String replyText, String exchange, String routingKey) {
            ...
            try {
                final String guid = SerializationUtils.getMapper().readTree(failedMessage).get("guid").asText();
                final EventPublisher publisher = new EventPublisher();
                publisher.setGuid(guid);
                if (NO_ROUTE.name().equalsIgnoreCase(replyText)) {
                    publisher.setStatus(NO_ROUTE);
                } else {
    ...
    private class RabbitConfirmCallback implements RabbitTemplate.ConfirmCallback {
        ...
        public void confirm(CorrelationData correlationData, boolean ack, String cause) {
            ...
            if (ack) {
                // flag instead
                publisherMapper.updateEventStatusByPrimaryKeyInCasMode(id, PENDING, DONE);
                // 或直接删除
                ...
                publisherMapper.updateEventStatusByPrimaryKeyInCasMode(id, PENDING, NOT_FOUND);
            }
        }
    }
